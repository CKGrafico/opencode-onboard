...
When the user provides a work item URL or says "implement the plan" or "I've added comments to the PR", **I own the full lifecycle**. I load the appropriate userstory skill and coordinate implementation as native subagent waves via `/plan-apply`.
...
- `I've added comments to the PR` or a PR URL in a feedback/fix request → load `ob-pullrequest` → classify feedback → fix via `/plan-apply` → update PR
